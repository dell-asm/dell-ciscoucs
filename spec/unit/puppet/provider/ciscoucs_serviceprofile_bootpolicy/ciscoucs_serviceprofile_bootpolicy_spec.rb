...
describe Puppet::Type.type(:ciscoucs_serviceprofile_bootpolicy).provider(:default) do
  ...
  context "when boot policy is applied to the service profile" do
      ...
      #When-Then
      @fixture.get_serviceprofile_organization.strip.length == 0 
      @fixture.get_serviceprofile_dn.strip.length == 0
      @fixture.get_serviceprofile_name.strip.length == 0
      expect {@fixture.provider.create}.to raise_error(Puppet::Error)
      end
      
      it "should raise error if all 3 parameters target organization, name and dn are blank" do
      #When-Then
      @fixture.get_bootpolicy_organization.strip.length == 0 
