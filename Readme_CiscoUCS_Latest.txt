...
The Cisco UCS Manager is the management system for all components in a Cisco UCS domain. Cisco UCS Manager runs within the fabric interconnect. You can use any of the interfaces available with this management service, to access, configure, administer, and monitor the network and server resources for all chassis connected to the fabric interconnect.
 
 
## Requirements
 The following are the requirements to interact with the ciscoucs:
 ...
 * Creation of service profile from existing server.
 * Creation of service profile from template.
 * Cloning of service profile.
 * Association of service profile.
 * Disassociation of service profile. 
 * Power on / off .
...
Boot policy must be included in a service profile, and that service profile must be associated with a server for it to take effect. If boot policy does not include in a service profile, the server uses the default settings in the BIOS to determine the boot order. 
